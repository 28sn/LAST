...
def start_scan():
    # حاول فتح الكاميرات المتاحة (من 0 إلى 4)
    cap = None
    for i in range(5):  # جرب حتى 5 كاميرات
        cap = cv2.VideoCapture(i)  # جرب الكاميرا ذات الرقم i
        if cap.isOpened():
            print(f"تم العثور على الكاميرا: {i}")
            break
        else:
            cap.release()

    if not cap or not cap.isOpened():
        return "❌ لا توجد كاميرا متاحة."

    # بدأ المسح من الكاميرا المتاحة
    while True:
        ret, frame = cap.read()
        if not ret:
